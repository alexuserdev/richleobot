...
async def main_operations(message: types.Message, state: FSMContext, _):
    await message.answer_sticker("CAACAgIAAxkBAAICVmD1mvmVX5-aB7P0jyVDHubhuuhkAAL_DQACo_ugSsQaq2gMY49PIAQ")
    msg = await message.answer(_("What would you like to do? 👩🏼‍🦰\n"
"\n"
...
async def join_send(call: types.CallbackQuery, state: FSMContext, _):
    ...
    for k, v in balances.items():
        if v:
            ...
            currencys.append(k)
    if flag:
        await call.message.edit_text(_("Choose currency to send"),
...
async def choosed_currency_to_send(call: types.CallbackQuery, state: FSMContext, _):
    currency = call.data.split(".")[1]
    data = await state.get_data()
    msg = await call.message.edit_text(_("Enter amount of {currency} you want to send to another user".format(
        currency=currency
    )),
...
async def entered_amount_to_send(message: types.Message, state: FSMContext, _):
    data = await state.get_data()
    ...
    try:
        ...
        if balance < amount:
            ...
            await message.answer(_("Enter user_id or user's nickname on Telegram"))
...
async def entered_user_id(message: types.Message, state: FSMContext, _):
    try:
        try:
        ...
        await message.answer(_("Enter correct user_id"))


async def confirm_send(call: types.CallbackQuery, state: FSMContext, _):
    data = await state.get_data()
    ...
    await dp.bot.send_message(user_id, _("You got {amount}{currency} from <a href='tg://user?id={user_id}'>{user_id}</a>".format(
...
async def join_request(call: types.CallbackQuery, _):
    await call.message.edit_text(_("To request funds, please specify amount and pick the currency you want to receive.\n\n"),
                                 reply_markup=OperationsKeyboard.request_methods())


async def choosed_request_method(call: types.CallbackQuery, state: FSMContext, _):
    method = call.data.split(".")[1]
    await call.message.edit_text(_("How much {method} you want to request".format(method=method)),
...
async def amount_entered(message: types.Message, state: FSMContext, _):
    ...
    try:
        ...
        await message.answer_photo(open(f"{message.chat.id}", "rb"),
                                   _("To request {amount} {method} choose person from your contacts\n\n <b>Or show them this QR.</b>".format(
                                       method=method, amount=amount
                                   )))
        os.remove(f"{message.chat.id}")
        await state.finish()
    except ValueError:
        msg = await message.answer(_("Invalid value"),
                                   reply_markup=OperationsKeyboard.request_methods(method))
        await state.update_data(last_msg=msg.message_id)


async def confirm_request(call: types.CallbackQuery, _):
    id = call.data.split(".")[1]
    await RequestsDb.delete_request(id)
    await call.message.edit_text(_("No much funds"))


async def eject_request(call: types.CallbackQuery, _):
    id = call.data.split(".")[1]
    await RequestsDb.delete_request(id)
    await call.message.edit_text(_("Transaction rejected."))
